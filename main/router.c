...
enum
{
    ...
    ROUTER_DHCP_LEASE_SECONDS = ROUTER_DHCP_LEASE_TIME_UNITS * CONFIG_LWIP_DHCPS_LEASE_UNIT,
...
static bool dhcp_lease_expired(const router_dhcp_lease_slot_t *lease, TickType_t now)
{
    return lease->active && (TickType_t)(now - lease->last_seen) >= dhcp_lease_duration_ticks();
}
...
static void record_dhcp_lease(router_lease_iface_t iface, uint32_t ip, const uint8_t mac[6])
{
    ...
    for (size_t i = 0; i < ROUTER_MAX_DHCP_LEASES; i++) {
        ...
        if (lease->iface == iface && (lease->ip == ip || memcmp(lease->mac, mac, sizeof(lease->mac)) == 0)) {
            match = (int)i;
            break;
        }
    }
...
static void dhcp_assigned_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    (void)arg;
    (void)event_base;
    ...
    record_dhcp_lease(lease_iface_from_netif(event->esp_netif), event->ip.addr, event->mac);
}
                                 ...
                                 bool advertise_dns, const char *name)
{
    ...
    ESP_ERROR_CHECK(esp_netif_dhcps_option(netif, ESP_NETIF_OP_SET, ESP_NETIF_IP_ADDRESS_LEASE_TIME, &lease_time,
                                           sizeof(lease_time)));
    ...
    ESP_ERROR_CHECK(
        esp_netif_dhcps_option(netif, ESP_NETIF_OP_SET, ESP_NETIF_REQUESTED_IP_ADDRESS, &pool, sizeof(pool)));

    dhcps_offer_t router_offer = advertise_gw ? OFFER_ROUTER : OFFER_START;
    ESP_ERROR_CHECK(esp_netif_dhcps_option(netif, ESP_NETIF_OP_SET, ESP_NETIF_ROUTER_SOLICITATION_ADDRESS,
                                           &router_offer, sizeof(router_offer)));

    dhcps_offer_t dns_offer = advertise_dns ? OFFER_DNS : OFFER_START;
    ESP_ERROR_CHECK(
        esp_netif_dhcps_option(netif, ESP_NETIF_OP_SET, ESP_NETIF_DOMAIN_NAME_SERVER, &dns_offer, sizeof(dns_offer)));
    ...
    ESP_LOGI(TAG, "%s: IP=" IPSTR " pool=.2-.%" PRIu32 " gw=%s dns=%s", name, IP2STR(&ip_info.ip), pool_end_host,
             advertise_gw ? "on" : "off", advertise_dns ? "on" : "off");
    return ESP_OK;
}
...
static uint8_t *append_static_route_option(uint8_t *opt, uint32_t subnet, uint32_t router)
{
...
static uint8_t *append_classless_route_option(uint8_t *opt, uint8_t option, uint32_t subnet, uint8_t prefix_len,
                                              uint32_t router)
{
...
static void append_route_options(uint8_t **opts, uint32_t route_subnet, uint8_t route_prefix_len, uint32_t router)
{
    uint8_t *opt = *opts;

    opt =
        append_classless_route_option(opt, DHCP_OPTION_CLASSLESS_STATIC_ROUTE, route_subnet, route_prefix_len, router);
    opt = append_classless_route_option(opt, DHCP_OPTION_MS_CLASSLESS_STATIC_ROUTE, route_subnet, route_prefix_len,
                                        router);
...
void router_append_dhcps_routes(struct netif *netif, u8_t state, u8_t **opts)
{
    ...
    if (router_subnets_overlap(s_cfg->usb_subnet, s_cfg->usb_prefix_len, s_cfg->wifi_subnet, s_cfg->wifi_prefix_len)) {
        return;
    }

    if (s_usb_netif && netif == esp_netif_get_netif_impl(s_usb_netif)) {
        esp_netif_ip_info_t ip_info;
        router_make_ip_info(s_cfg->usb_subnet, s_cfg->usb_prefix_len, true, &ip_info);
        append_route_options(opts, s_cfg->wifi_subnet, s_cfg->wifi_prefix_len, ip_info.ip.addr);
    } else if (s_wifi_netif && netif == esp_netif_get_netif_impl(s_wifi_netif)) {
        esp_netif_ip_info_t ip_info;
        router_make_ip_info(s_cfg->wifi_subnet, s_cfg->wifi_prefix_len, true, &ip_info);
        append_route_options(opts, s_cfg->usb_subnet, s_cfg->usb_prefix_len, ip_info.ip.addr);
    }
}
...
s16_t router_observe_dhcps_state(struct dhcps_msg *msg, u16_t len, s16_t state)
{
    ...
    if (state == DHCPS_STATE_RELEASE_LOCAL || state == DHCPS_STATE_DECLINE_LOCAL || state == DHCPS_STATE_NAK_LOCAL) {
        uint32_t ciaddr = 0;
...
esp_err_t router_start(esp_netif_t *usb_netif, esp_netif_t *wifi_netif, const taplink_config_t *cfg)
{
    ...
    s_cfg = cfg;
    clear_dhcp_leases();
    ESP_ERROR_CHECK(esp_event_handler_register(IP_EVENT, IP_EVENT_AP_STAIPASSIGNED, dhcp_assigned_handler, NULL));

    if (router_subnets_overlap(cfg->usb_subnet, cfg->usb_prefix_len, cfg->wifi_subnet, cfg->wifi_prefix_len)) {
        ESP_LOGW(TAG, "USB and WiFi subnets overlap; routed forwarding needs "
                      "distinct subnets");
    }
...
#else
    ESP_LOGE(TAG, "CONFIG_LWIP_IP_FORWARD is disabled; clients can reach "
                  "gateways but not the other subnet");
#endif
