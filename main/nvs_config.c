...
esp_err_t config_save(const netlink_config_t *cfg)
{
    ...
    for (int i = 0; i < NETLINK_MAX_PORT_FORWARDS; i++) {
        char key[16];
        snprintf(key, sizeof(key), "pf%d_enabled", i);
        nvs_set_u8(handle, key, cfg->port_forwards[i].enabled);
        snprintf(key, sizeof(key), "pf%d_lport", i);
        nvs_set_u16(handle, key, cfg->port_forwards[i].listen_port);
        snprintf(key, sizeof(key), "pf%d_tip", i);
        nvs_set_u32(handle, key, cfg->port_forwards[i].target_ip);
        snprintf(key, sizeof(key), "pf%d_tport", i);
        nvs_set_u16(handle, key, cfg->port_forwards[i].target_port);
    }
