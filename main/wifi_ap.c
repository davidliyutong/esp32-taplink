...
static void wifi_ap_event_handler(void *arg, esp_event_base_t base, int32_t event_id, void *event_data)
{
    switch (event_id) {
    ...
    case WIFI_EVENT_AP_STOP:
        ...
        if (s_restart_attempts < 3) {
            s_restart_attempts++;
            TimerHandle_t t = xTimerCreate("ap_rst", pdMS_TO_TICKS(2000), pdFALSE, NULL, ap_restart_timer_cb);
            if (t)
                xTimerStart(t, 0);
        } else {
    ...
    case WIFI_EVENT_AP_STACONNECTED: {
        ...
        s_sta_count++;
        ESP_LOGI(TAG, "Station " MACSTR " connected (aid=%d, total=%d)", MAC2STR(e->mac), e->aid, s_sta_count);
        break;
    }
    case WIFI_EVENT_AP_STADISCONNECTED: {
        ...
        if (s_sta_count > 0)
            s_sta_count--;
        ESP_LOGI(TAG, "Station " MACSTR " disconnected (reason=%d, total=%d)", MAC2STR(e->mac), e->reason, s_sta_count);
        break;
    }
    }
}
...
esp_netif_t *wifi_ap_create(const netlink_config_t *cfg)
{
    ...
    router_make_ip_info(cfg->wifi_subnet, cfg->wifi_prefix_len, cfg->dhcp_gw_enabled, &ip_info);
    wifi_cfg.ip_info = &ip_info;

    esp_netif_t *netif = esp_netif_create_wifi(WIFI_IF_AP, &wifi_cfg);
    assert(netif);
    esp_wifi_set_default_wifi_ap_handlers();
    ESP_ERROR_CHECK(router_configure_dhcps(netif, cfg->wifi_subnet, cfg->wifi_prefix_len, cfg->dhcp_gw_enabled,
                                           cfg->dhcp_dns_enabled, "WiFi AP"));
...
esp_err_t wifi_ap_start(const netlink_config_t *cfg)
{
    ...
    size_t ssid_len = strnlen(cfg->wifi_ssid, sizeof(wifi_config.ap.ssid));
    memcpy(wifi_config.ap.ssid, cfg->wifi_ssid, ssid_len);
    wifi_config.ap.ssid_len = ssid_len;

    size_t pass_len = strnlen(cfg->wifi_password, sizeof(wifi_config.ap.password) - 1);
    if (pass_len < 8) {
        wifi_config.ap.authmode = WIFI_AUTH_OPEN;
        ESP_LOGW(TAG, "Password too short, using open auth");
    } else {
        memcpy(wifi_config.ap.password, cfg->wifi_password, pass_len);
    }
    ...
    ESP_LOGI(TAG, "WiFi AP started: SSID=%s ch=%d tx=%.1fdBm", cfg->wifi_ssid, channel, tx_power * 0.25f);
    return ESP_OK;
}
